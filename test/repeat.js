...
describe('测试ms-repeat', function () {
    it("sync", function () {
        ...
        str = str.replace(/ms-scan-\d+="[^"]+"/g, "").replace(/<!--\w+\d+(:end)?-->/g, "").replace(/<\w+\s+>/g, "")
        console.log(str)
        ...
        var areas = avalon.getElementsByTagName(dom, "area")
        expect(dom(areas[0]).attr("attr")).to.be("a")
        expect(dom(areas[1]).attr("attr")).to.be("b")
        expect(dom(areas[2]).attr("attr")).to.be("c")
